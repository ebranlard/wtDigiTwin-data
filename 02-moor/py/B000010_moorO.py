...
def forcing(t,q=None,qd=None,p=None,u=None,z=None):
    ...
    p:  parameters, dictionary with keys: ['KM_44', 'M_B', 'g', 'x_BG', 'z_BG']
    ...
    if z is not None:
        ...
        qd = z[int(len(z)/2): ] 
    FF = np.zeros((1,1))
    FF[0,0] = -p['KM_44']*q[0]+p['M_B']*p['g']*p['x_BG']*cos(q[0])+p['M_B']*p['g']*p['z_BG']*sin(q[0])
    return FF
...
def K_lin(q=None,qd=None,p=None,u=None,z=None):
    ...
    if z is not None:
        ...
        qd = z[int(len(z)/2): ] 
    KK = np.zeros((1,1))
    KK[0,0] = p['KM_44']+p['M_B']*p['g']*p['x_BG']*sin(q[0])-p['M_B']*p['g']*p['z_BG']*cos(q[0])
    return KK

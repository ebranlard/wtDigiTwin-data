...
def forcing(t,q=None,qd=None,p=None,u=None,z=None):
    ...
    p:  parameters, dictionary with keys: ['KM_00', 'KM_04', 'KM_44', 'M_B', 'g', 'x_BG', 'z_BG', 'z_BM']
    ...
    if z is not None:
        ...
        qd = z[int(len(z)/2): ] 
    FF = np.zeros((2,1))
    FF[0,0] = -p['KM_00']*q[0]-p['KM_04']*q[1]+p['M_B']*p['x_BG']*qd[1]**2*cos(q[1])+p['M_B']*p['z_BG']*qd[1]**2*sin(q[1])
    FF[1,0] = -p['KM_00']*p['z_BM']*q[0]*cos(q[1])-p['KM_04']*p['z_BM']*q[1]*cos(q[1])-p['KM_04']*q[0]-p['KM_44']*q[1]+p['M_B']*p['g']*p['x_BG']*cos(q[1])+p['M_B']*p['g']*p['z_BG']*sin(q[1])
    return FF
...
def K_lin(q=None,qd=None,p=None,u=None,z=None):
    ...
    KK = np.zeros((2,2))
    KK[0,0] = p['KM_00']
    KK[0,1] = p['KM_04']+p['M_B']*p['x_BG']*qd[1]**2*sin(q[1])-p['M_B']*p['z_BG']*qd[1]**2*cos(q[1])
    KK[1,0] = p['KM_00']*p['z_BM']*cos(q[1])+p['KM_04']
    KK[1,1] = p['KM_04']*p['z_BM']*cos(q[1])+p['KM_44']+p['M_B']*p['g']*p['x_BG']*sin(q[1])-p['M_B']*p['g']*p['z_BG']*cos(q[1])+p['z_BM']*(-p['KM_00']*q[0]-p['KM_04']*q[1])*sin(q[1])
    return KK

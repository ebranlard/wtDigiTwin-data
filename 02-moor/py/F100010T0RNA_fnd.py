...
def forcing(t,q=None,qd=None,p=None,u=None,z=None):
    ...
    p:  parameters, dictionary with keys: ['KM_00', 'KM_04', 'KM_44', 'L_T', 'M_F', 'M_RNA', 'M_T', 'g', 'x_RNAG', 'z_FG', 'z_RNAG', 'z_T0', 'z_TG']
    ...
    if z is not None:
        ...
        qd = z[int(len(z)/2): ] 
    FF = np.zeros((2,1))
    FF[0,0] = -p['KM_00']*q[0]-p['KM_04']*q[1]+p['L_T']*p['M_RNA']*qd[1]**2*sin(q[1])+p['M_F']*p['z_FG']*qd[1]**2*sin(q[1])+p['M_RNA']*p['x_RNAG']*qd[1]**2*cos(q[1])+p['M_RNA']*p['z_RNAG']*qd[1]**2*sin(q[1])+p['M_T']*p['z_TG']*qd[1]**2*sin(q[1])+u['F_hx'](t,q,qd)
    FF[1,0] = -p['KM_04']*q[0]-p['KM_44']*q[1]+p['L_T']*p['M_RNA']*p['g']*sin(q[1])+p['M_F']*p['g']*p['z_FG']*sin(q[1])+p['M_RNA']*p['g']*p['x_RNAG']*cos(q[1])+p['M_RNA']*p['g']*p['z_RNAG']*sin(q[1])+p['M_T']*p['g']*p['z_TG']*sin(q[1])+p['z_T0']*u['F_hx'](t,q,qd)*cos(q[1])-p['z_T0']*u['F_hz'](t,q,qd)*sin(q[1])
    return FF
...
def K_lin(q=None,qd=None,p=None,u=None,z=None):
    ...
    KK = np.zeros((2,2))
    KK[0,0] = p['KM_00']
    KK[0,1] = p['KM_04']-p['L_T']*p['M_RNA']*qd[1]**2*cos(q[1])-p['M_F']*p['z_FG']*qd[1]**2*cos(q[1])+p['M_RNA']*p['x_RNAG']*qd[1]**2*sin(q[1])-p['M_RNA']*p['z_RNAG']*qd[1]**2*cos(q[1])-p['M_T']*p['z_TG']*qd[1]**2*cos(q[1])
    KK[1,0] = p['KM_04']
    KK[1,1] = p['KM_44']-p['L_T']*p['M_RNA']*p['g']*cos(q[1])-p['M_F']*p['g']*p['z_FG']*cos(q[1])+p['M_RNA']*p['g']*p['x_RNAG']*sin(q[1])-p['M_RNA']*p['g']*p['z_RNAG']*cos(q[1])-p['M_T']*p['g']*p['z_TG']*cos(q[1])-p['z_T0']*(-u['F_hx']*sin(q[1])-u['F_hz']*cos(q[1]))
    return KK
